...
class HSplit(_Split):
    ...
    def preferred_width(self, max_available_width: int) -> Dimension:
        ...
        if self.children:
            dimensions = [c.preferred_width(
                max_available_width) for c in self.children]
    ...
    def _divide_heights(self, write_position: WritePosition) -> Optional[List[int]]:
        ...
        # Calculate heights.
        dimensions = [c.preferred_height(width, height)
...
class Window(Container):
    ...
    def _write_to_screen_at_index(
        ...
        left_margin_widths = [self._get_margin_width(
            m) for m in self.left_margins]
        right_margin_widths = [self._get_margin_width(
        ...
        for m, width in zip(self.left_margins, left_margin_widths):
            if width > 0:  # (ConditionalMargin returns a zero width. -- Don't render.)
                ...
                # Copy and shift X.
                self._copy_margin(margin_content, screen,
                                  write_position, move_x, width)
                move_x += width
    ...
    def _copy_margin(
        ...
        self._copy_body(margin_content, new_screen,
                        margin_write_position, 0, width)
    ...
    def _scroll_when_linewrapping(
        ...
        if line_height > height - scroll_offsets_top:
            ...
            self.vertical_scroll_2 = min(
                ...
                line_height
                # Avoid blank lines at the bottom when scolling up again.
                - height,
        ...
        self.vertical_scroll = max(
            self.vertical_scroll, min(
                topmost_visible, get_min_vertical_scroll())
        )
        self.vertical_scroll = min(
    ...
    def _scroll_without_linewrapping(
        ...
        def do_scroll(
            ...
            scroll_offset_end = int(
                min(scroll_offset_end, window_size /
                    2, content_size - 1 - cursor_pos)
            )
            ...
            if current_scroll < (cursor_pos + 1) - window_size + scroll_offset_end:
                current_scroll = (cursor_pos + 1) - \
                    window_size + scroll_offset_end
        ...
        if self.get_line_prefix:
            current_line_prefix_width = fragment_list_width(
                to_formatted_text(self.get_line_prefix(
    ...
    def _scroll_down(self) -> None:
        ...
        if should_scroll:
            self.vertical_scroll += 1
            # if info.cursor_position.y <= info.configured_scroll_offsets.top: #::::ADDED::::
            self.content.move_cursor_down()
