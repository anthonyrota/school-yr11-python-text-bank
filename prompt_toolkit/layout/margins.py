...
class ScrollbarMargin(Margin):
    ...
    def create_margin(
        ...
        try:
            lines_above = sum(map(window_render_info.get_height_for_line, range(
                window_render_info.vertical_scroll)))
            height = sum(
                map(window_render_info.get_height_for_line, range(window_render_info.content_height)))

            display_line_len = len(window_render_info.displayed_lines)

            fraction_above = lines_above / float(height)
            fraction_visible = display_line_len / float(height)

            scrollbar_height = int(
                min(window_height, max(1, window_height * fraction_visible))) - (0 if lines_above+display_line_len == height else 1)
            scrollbar_top = int(
                window_height * fraction_above)
            ...
            if scrollbar_top == 0 and scrollbar_height == window_height:
                return []
            ...
            for i in range(window_height):
                if is_scroll_button(i):
                    if not is_scroll_button(i + 1):
                        ...
                        result.extend([('', ' '), (scrollbar_button_end, " ")])
                    else:
                        result.extend([('', ' '), (scrollbar_button, " ")])
                else:
                    if is_scroll_button(i + 1):
                        result.extend(
                            [('', ' '), (scrollbar_background_start, " ")])
                    else:
                        result.extend([('', ' '), (scrollbar_background, " ")])
                result.append(("", "\n"))
